...
namespace CEA_CR.Data.Manager
{
    ...
    /// 学生管理类
    /// </summary>
    public class StudentInfoManager : ManagerBase
    {
        /// <summary>
        /// 插入一条记录
        /// </summary>
        ...
        /// <returns></returns>
        public void Insert(StudentInfo entity)
        {
            IDbSession session = SessionFactory.CreateSession();
            try
            {
                session.BeginTrans();
                Repository.Insert<StudentInfo>(session.Connection, entity, session.Transaction);
                session.Commit();
            }
        ...
        public void Update(StudentInfo entity)
        {
            ...
            {
                session.BeginTrans();
                Repository.Update<StudentInfo>(session.Connection, entity, session.Transaction);
                session.Commit();
            }
            catch (System.Exception)
            {
        ...
        public StudentInfo GetFirstOrDefault(string id)
        {
            string sql = @"select * from StudentInfo where StudentID=@id";
            return Repository.Query<StudentInfo>(sql, new { id = id }).FirstOrDefault();
        }
        public List<StudentInfo> GetSearch(Dictionary<string, string> para, string sort, string order, int offset, int pageSize, out int total)
        {
            StringBuilder commandsb = new StringBuilder("from StudentInfo where ");
            ...
            return Repository.Query<StudentInfo>(querySql).ToList();
        }
    }
}